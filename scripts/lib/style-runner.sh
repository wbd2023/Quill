#!/bin/bash
#
# tools/scripts/lib/style-runner.sh
...
source "$STYLE_RUNNER_LIB_DIR/style-common.sh"
	...
	"$STYLE_RUNNER_EXECUTOR")
		...
		"$STYLE_RUNNER_TARGET_AST_TOOLS")
			(
				...
				GOCACHE="$GO_BUILD_CACHE" \
					go run ./cmd/stylecheck \
					"$PROJECT_ROOT/tools/stylecheck/cmd/stylecheck" \
					"$PROJECT_ROOT/tools/stylecheck/internal/checker" \
