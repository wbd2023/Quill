#!/bin/bash
#
# tools/style/entrypoints/check-style.sh
# Master STYLE.md compliance runner.
#
# Usage:
#	./tools/style/entrypoints/check-style.sh [--verbose] [--scope app|tools|all] \
...
STYLE_DIR="$(cd "$ENTRYPOINT_DIR/.." && pwd)"
# shellcheck source=tools/style/internal/common.sh
source "$STYLE_DIR/internal/common.sh"
# shellcheck source=tools/style/internal/runner.sh
source "$STYLE_DIR/internal/runner.sh"
# shellcheck source=tools/style/internal/registry.sh
source "$STYLE_DIR/internal/registry.sh"

# Runner helpers expect SCRIPT_DIR as the base directory for check targets.
SCRIPT_DIR="$STYLE_DIR"
