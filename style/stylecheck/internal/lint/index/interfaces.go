...
// CollectInterfaces records interface method order from application port packages.
func CollectInterfaces(
	...
	if !paths.IsApplicationPortPath(path) {
		return
	}
...
func CollectMockMethods(
	...
	for _, declaration := range file.Decls {
		...
		receiver := syntax.ReceiverTypeName(funcDecl.Recv.List[0].Type)
		if receiver == "" {
			continue
		}
