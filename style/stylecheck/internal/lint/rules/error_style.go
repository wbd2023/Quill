...
func CheckGoErrorHandlingStyle(
	...
	ast.Inspect(file, func(node ast.Node) bool {
		...
		case selectorExpression.Sel.Name == "Errorf" && fmtImportAliases[packageIdentifier.Name]:
			message, found := syntax.ExtractStringLiteral(callExpression.Args[0])
			if found {
				violations = append(
					violations,
	...
	if isTestFile || paths.IsDomainErrorsFilePath(path) {
		return violations
	}
...
func CheckAdapterErrorWrapping(
	...
	if isTestFile || !paths.IsConcreteInfraPath(path) {
		return nil
	}
