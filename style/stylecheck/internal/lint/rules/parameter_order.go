...
func CheckParamOrder(fileSet *token.FileSet, file *ast.File) (violations []report.Violation) {
...
func CheckConstructorOrder(fileSet *token.FileSet, file *ast.File) (violations []report.Violation) {
	ast.Inspect(file, func(node ast.Node) bool {
		funcDecl, ok := node.(*ast.FuncDecl)
		if !ok || funcDecl.Type.Params == nil {
			return true
...
func classifyParam(field *ast.Field) (category int) {
	typeName := syntax.TypeString(field.Type)
