#!/bin/bash
#
# tools/style/internal/registry.sh
...
# shellcheck source=tools/style/internal/registry-constants.sh
source "$STYLE_REGISTRY_LIB_DIR/registry-constants.sh"
...
REGISTRY_TABLE_FILENAME="registry.table"
...
STYLE_REGISTRY_STYLE_DIR="$(cd "$STYLE_REGISTRY_LIB_DIR/.." && pwd)"
...
style_registry_is_valid_target() {
	...
	"$REGISTRY_RUNNER_SCRIPT" | "$REGISTRY_RUNNER_SCRIPT_SCOPE")
		...
		[ -f "$STYLE_REGISTRY_STYLE_DIR/$target" ]
		return
		;;
