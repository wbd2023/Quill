...
func decodeFileSets(schemas map[string]schemaFileSetConfig) (fileSets FileSets) {
	fileSets = make(FileSets, 0, len(schemas))
	for _, name := range sortedMapKeys(schemas) {
		fileSet := schemas[name]
		fileSets = append(fileSets, FileSetConfig{
			Name: name,
...
func encodeFileSets(fileSets FileSets) (schemas map[string]schemaFileSetConfig) {
	if fileSets == nil {
		return nil
	}
