...
func configureProcessTree(command *exec.Cmd) (state *killState, afterStart func(cmd *exec.Cmd) error, release func(), err error) {
	...
	return state, bindSuspendedChild(job, state), func() { _ = windows.CloseHandle(job) }, nil
}
...
func cancelJobTree(command *exec.Cmd, job windows.Handle, state *killState) (cancel func() (err error)) {
...
// born inside the job.
func bindSuspendedChild(job windows.Handle, state *killState) func(cmd *exec.Cmd) error {
