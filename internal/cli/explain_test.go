...
func TestExplainActiveRule(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "rule:profile/enforcement-levels",
	})
	if exitCode != 0 {
...
func TestExplainRejectsMissingSubject(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{"explain", "--repository-root", repo})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}

	if !strings.Contains(stderr.String(), `expected "<subject>"`) {
...
func TestExplainRejectsSubjectWithoutKind(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "profile/enforcement-levels",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}
...
func TestExplainValidatesSubjectBeforeRepositoryDiscovery(t *testing.T) {
	t.Chdir(t.TempDir())

	tool, stdout, stderr := newTestCLI()
	exitCode := tool.Run(context.Background(), []string{"explain", "not-a-subject"})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}
	if stdout.Len() != 0 {
		t.Fatalf("expected no stdout for invalid subject, got %q", stdout.String())
	}
	if !strings.Contains(stderr.String(), "invalid subject") {
		t.Fatalf("expected invalid-subject error, got %q", stderr.String())
	}
...
func TestExplainRejectsUnsupportedKind(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "pack:project",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}
...
func TestExplainRejectsEmptyRuleID(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "rule:",
	})
	if exitCode != usageExitCode {
...
func TestExplainUnknownRuleIsInvalidArgument(t *testing.T) {
	t.Parallel()
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "rule:does/not-exist",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d for unknown rule, got %d", usageExitCode, exitCode)
	}
...
func TestExplainDeclaredInactiveRuleIsInvalidArgument(t *testing.T) {
	t.Parallel()
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "rule:go/file-shape",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d for inactive rule, got %d", usageExitCode, exitCode)
	}
...
func TestExplainJSONEnvelope(t *testing.T) {
	...
	if rule.Pack.ID != "project" || rule.Pack.Provenance != "shipped" {
		t.Fatalf("pack = %+v, want shipped project Pack", rule.Pack)
	}
	if len(rule.Pack.Policy) == 0 {
		t.Fatalf("active Pack Policy is missing: %+v", rule.Pack)
	}
	if rule.Binding.Enforcement == "" || rule.Binding.Scope == "" {
		t.Fatalf("active Rule missing binding: %+v", rule)
	}
	if len(rule.Binding.Requirements) == 0 {
...
func TestExplainJSONUnknownRuleIsInvalidArgument(t *testing.T) {
	t.Parallel()
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--format", "json", "--repository-root", repo, "rule:does/not-exist",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}
...
func TestExplainJSONInvalidSubjectIsInvalidArgument(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--format", "json", "--repository-root", repo, "pack:project",
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d, got %d", usageExitCode, exitCode)
	}
...
func TestExplainLaunchesNoToolProcess(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"explain", "--repository-root", repo, "rule:go/lint",
	})
	if exitCode != 0 {
