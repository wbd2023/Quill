...
func TestInitCreatesProfileAndStyleGuide(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{"init", "--repo-root", root})
	if exitCode != 0 {
...
func TestInitRejectsUnknownPreset(t *testing.T) {
	...
	exitCode := tool.Run(context.Background(), []string{
		"init", "--preset", "bogus", "--repo-root", root,
	})
	if exitCode != usageExitCode {
		t.Fatalf("expected usage exit %d for unknown preset, got %d", usageExitCode, exitCode)
	}

	if !strings.Contains(stderr.String(), `unsupported preset "bogus"`) {
		t.Fatalf("expected unsupported-preset error, got %q", stderr.String())
	}
...
func TestParsePresetAcceptsMinimalAndDefault(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{name: "empty defaults to minimal", value: "", want: defaultPreset},
		{name: "explicit minimal", value: defaultPreset, want: defaultPreset},
		{name: "unknown rejected", value: "go", wantErr: true},
		{name: "blank rejected", value: "   ", want: defaultPreset},
	}
	...
	for _, command := range cases {
		t.Run(command.name, func(t *testing.T) {
			...
			preset, err := parsePreset(command.value)
			if command.wantErr {
				if err == nil {
					t.Fatalf("expected error for %q", command.value)
				}
				return
			}

			if err != nil {
				t.Fatalf("unexpected error for %q: %v", command.value, err)
			}

			if preset != command.want {
				t.Fatalf("preset = %q, want %q", preset, command.want)
			}
		})
	}
}
...
func TestInitWritesNoOutputToStdoutOnError(t *testing.T) {
	...
	_ = tool.Run(context.Background(), []string{"init", "--repo-root", root})
...
func TestInitRejectsDanglingSymlink(t *testing.T) {
	...
	if _, err := os.Lstat(filepath.Join(root, profileFileName)); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("quill.toml must not be written, got %v", err)
	}
}

func TestWriteExclusiveRefusesExistingFile(t *testing.T) {
	t.Parallel()
	...
	if err := writeExclusive(path, "replaced"); !errors.Is(err, os.ErrExist) {
		t.Fatalf("writeExclusive on existing file must fail with ErrExist, got %v", err)
	}
	...
	if string(got) != "original" {
		t.Fatalf("existing file must not be truncated or altered, got %q", string(got))
	}
}

func TestWritePolicyFilesRollsBackOnSecondFailure(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	stylePath := filepath.Join(root, styleFileName)
	// A profile path whose parent directory does not exist fails the exclusive create, so the
	// already-written STYLE.md must be rolled back.
	profilePath := filepath.Join(root, "missing", profileFileName)

	if err := writePolicyFiles(stylePath, profilePath, "style", "profile"); err == nil {
		t.Fatal("expected writePolicyFiles to fail when the profile parent is missing")
	}

	if _, err := os.Lstat(stylePath); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("STYLE.md must be rolled back after the profile write fails, got %v", err)
	}
}

func TestPolicyFilesOccupiedDetectsSymlink(t *testing.T) {
	t.Parallel()
	...
	occupied, err := policyFilesOccupied(link)
	if err != nil {
		t.Fatalf("policyFilesOccupied on symlink: %v", err)
	}
	if !occupied {
		t.Fatal("a dangling symlink must be reported as occupied")
	}

	occupied, err = policyFilesOccupied(filepath.Join(root, "absent"))
	if err != nil {
		t.Fatalf("policyFilesOccupied on absent path: %v", err)
	}
	if occupied {
		t.Fatal("an absent path must not be reported as occupied")
	}
}
