...
func CheckErrorHandlingStyle(
	...
	ast.Inspect(file, func(node ast.Node) bool {
		...
		case selectorExpr.Sel.Name == "Errorf" && fmtImportAliases[pkgIdent.Name]:
			message, found := stringLiteralValue(callExpr.Args[0])
			if found {
				violations = append(
					violations,
	...
	if isTestFile || classifier.HasClass(path, rulepack.PathClassDomainErrors) {
		return violations
	}
			...
			for _, name := range valueSpec.Names {
				...
				violations = append(violations, Violation{
					...
					Message: fmt.Sprintf(
						...
						classifier.FirstPattern(rulepack.PathClassDomainErrors),
					),
				})
			}
		}
...
func CheckAdapterErrorWrapping(
	...
	if isTestFile || !classifier.HasClass(path, rulepack.PathClassConcreteInfra) {
		return nil
	}
