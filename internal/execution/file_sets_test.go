...
func TestCollectFileSetFilesUsesProfileDefinedScopeFilters(t *testing.T) {
	root := t.TempDir()
	profiles.Write(t, root, scopedFileSetConfig(t))

	readme := testutil.WriteFile(t, root, "README.md", "# App\n")
	testutil.WriteFile(t, root, "internal/guide.md", "# Internal\n")
	toolsReadme := testutil.WriteFile(t, root, "tools/README.md", "# Tools\n")

	context := testContext(t, root, style.Scope("app"))
...
func TestCollectFileSetFilesReturnsEmptySetWhenScopedIncludesDoNotOverlap(t *testing.T) {
	...
	testutil.WriteFile(t, root, "README.md", "# App\n")

	context := testContext(t, root, style.Scope("tools"))
...
func TestCollectLineLengthFileSetCoversTextFiles(t *testing.T) {
	root := t.TempDir()
	profiles.Write(t, root, profiles.Self(t))
	makefile := testutil.WriteFile(t, root, "Makefile", "all:\n\t@true\n")
	config := testutil.WriteFile(t, root, "style.local.toml", "enabled = true\n")
	checksum := testutil.WriteFile(t, root, "go.sum", strings.Repeat("x", 120)+"\n")
...
func TestCollectFileSetFilesSelectsPrivacyDocumentAndExcludesUnlistedRootMarkdown(t *testing.T) {
	...
	privacy := testutil.WriteFile(t, root, "CONTRIBUTOR_PRIVACY.md", "# Contributor Privacy\n")
	unlisted := testutil.WriteFile(t, root, "UNLISTED.md", "# Unlisted\n")
