...
func TestDecodeConfigReadsEncodedConfig(t *testing.T) {
	...
	got, err := textpolicy.DecodeConfig(textpolicy.EncodeConfig(expected))
	if err != nil {
		t.Fatalf("DecodeConfig: %v", err)
	}
...
func TestValidateConfigRejectsInvalidSectionHeaderPolicy(t *testing.T) {
	...
	err := textpolicy.ValidateConfig(config)
	if err == nil {
		t.Fatal("expected validation error")
	}
...
func baselineConfig() (config textpolicy.Config) {
	return textpolicy.Config{
