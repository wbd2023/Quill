...
func CheckStructureOrder(
	...
	for _, declaration := range file.Decls {
		category := classifyTopLevelDecl(declaration)
		if category == declUnknown {
			continue
		}

		seenCategories[category] = true
		if category < highestSeenCategory {
			violations = append(violations, analysis.Violation{
				Position: fileSet.Position(declaration.Pos()),
				Rule:     analysis.DiagnosticFileOrder,
				Message: fmt.Sprintf(
					"declaration group %q appears after %q",
					declCategoryName(category),
