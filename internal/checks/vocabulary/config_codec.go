...
// DecodeConfig decodes the vocabulary pack config subtree.
...
// ValidatePackConfig validates the raw vocabulary pack config subtree.
func ValidatePackConfig(pack policy.PackConfig) (err error) {
	_, err = DecodeConfig(pack)
	return err
}

// EncodeConfig encodes config as a raw vocabulary pack config subtree.
