...
func TestListJSONCarriesOnlySelectedSection(t *testing.T) {
	...
	for _, command := range cases {
		t.Run(command.name, func(t *testing.T) {
			...
			if err := WriteList(&buffer, "list", FormatJSON, command.result); err != nil {
...
func TestListRuleJSONOmitsEmptyEnforcementForInactiveRules(t *testing.T) {
	...
	if err := WriteList(&buffer, "list", FormatJSON, result); err != nil {
		t.Fatalf("WriteList: %v", err)
	}
...
func TestExplainJSONOmitsFixWhenAbsent(t *testing.T) {
	...
	if err := WriteExplain(&buffer, "explain", FormatJSON, result); err != nil {
...
func TestExplainTextRendersPackConfigDeterministically(t *testing.T) {
	t.Parallel()
	...
	var buffer bytes.Buffer
	if err := WriteExplain(&buffer, "explain", FormatText, result); err != nil {
		t.Fatalf("WriteExplain: %v", err)
	}

	output := buffer.String()
	if !strings.Contains(output, "Pack config") {
		t.Fatalf("text explain must render a Pack config section:\n%s", output)
	}
	...
	if alphaIdx < 0 || zebraIdx < 0 || alphaIdx > zebraIdx {
		t.Fatalf("config keys must render sorted (alpha before zebra):\n%s", output)
	}
	if inner1Idx < 0 || inner2Idx < 0 || inner1Idx > inner2Idx {
		t.Fatalf("nested config keys must render sorted (inner1 before inner2):\n%s", output)
	}
}

func TestExplainTextOmitsPackConfigWhenEmpty(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(buffer.String(), "Pack config") {
		t.Fatalf("empty pack config must not render a section:\n%s", buffer.String())
	}
}
